...
module.exports = {
	 ...
	 * @param {bool}   renaameOriginal  boolean as to weather
	 * @param {string} namingConvention date|custom|original
	 * @param {string} seperator        config seperator option, aka _
	 */
	renameOriginal: function(oldName, renameOriginal, namingConvention, seperator) {
		if (renameOriginal && namingConvention == 'date') {
			var ext = this.getFileExt(oldName);
			var fname = this.getFileRoot(oldName);
			return this.normalizeName(fname) + seperator + String(new Date().getTime()) 
				+ '.' + ext;
		} else {
